...
export const Create = () => {
  ...
  const sentForm = (fh: formHTML) => {
    ...
    if (currentForm.checkValidity() == false) {
      ...
      // Hook!
      const newCode = arrVehicles.length + 1;
      obj._id = newCode;
      obj._photoB64 = imgBs64;

      arrVehicles.push(obj);
      setInProgress(false);
      navigate("/catalog");
    }
  };
  ...
  let { _plate, _model, _brand, _photoName, dobleLink, obj } = useForm<Vehicle>(
    new Vehicle(0, "", "", "", "", "")
  );
      ...
      <div className="container p-5 col-md-5">
        <div className="card bg-dark text-white text-next border">
          <Form validated={inProgress} onSubmit={sentForm} noValidate>
            <div className="card-header border bg-light text-dark"><strong>Add new vehicle</strong></div>
